...
pub struct LeftShiftRegister {
	...
	pub fn step(&mut self) -> u8{
		...
		// println!("or seed : {:#010b}",or_seed );
		
		let seed_bool : bool = (or_seed & self.buffer) == or_seed;
		// println!("or_seed & self.buffer : {:#010b}",or_seed & self.buffer );
		
		let first_bool : bool = (0x80 & self.buffer) == 0x80;
		// println!("0x80 & self.buffer : {:#010b}",0x80 & self.buffer );
		
		let mut return_bit :u8 = 0;

		self.buffer = self.buffer << 1;
		// println!("self.buffer << 1 : {:#010b}",self.buffer );

		if seed_bool != first_bool{
			return_bit = 1
		}
	    self.buffer |= return_bit;
		// println!("self.buffer |= return_bit : {:#010b}",self.buffer );
